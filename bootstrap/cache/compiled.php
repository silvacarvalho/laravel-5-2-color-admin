...
class Gate implements GateContract
{
    ...
    {
        $arguments = array_merge(array($user, $ability), array($arguments));
    ...
    {
        $arguments = array_merge(array($user, $ability, $result), array($arguments));
...
trait ResetsPasswords
{
    ...
    {
        return array('token' => 'required', 'email' => 'required|email', 'password' => 'required|confirmed|min:6');
    }
...
abstract class AbstractPaginator implements Htmlable
{
    ...
    public function url($page)
    {
        ...
        return $this->path . (str_contains($this->path, '?') ? '&' : '?') . http_build_query($parameters, null, '&') . $this->buildFragment();
    }
    public function previousPageUrl()
    {
...
class Router implements RegistrarContract
{
    ...
    public function resourceParameters(array $parameters = array())
    {
...
class Route
{
    ...
    protected function compileRoute()
    {
        ...
        $this->compiled = (new SymfonyRoute($uri, $optionals, $this->wheres, array(), $this->domain() ?: ''))->compile();
    }
...
abstract class Model implements ArrayAccess, Arrayable, Jsonable, JsonSerializable, QueueableEntity, UrlRoutable
{
    ...
    {
        $modelScopes = Arr::get(static::$globalScopes, static::class, array());
        if (is_string($scope)) {
    ...
    {
        return Arr::get(static::$globalScopes, static::class, array());
    }
